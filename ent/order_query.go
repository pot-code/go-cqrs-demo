...
type OrderQuery struct {
	...
	// eager-loading edges.
	withOrderItems *OrderLineItemQuery
...
func (oq *OrderQuery) QueryOrderItems() *OrderLineItemQuery {
...
func (oq *OrderQuery) WithOrderItems(opts ...func(*OrderLineItemQuery)) *OrderQuery {
	query := &OrderLineItemQuery{config: oq.config}
...
func (oq *OrderQuery) sqlAll(ctx context.Context) ([]*Order, error) {
	...
	if query := oq.withOrderItems; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.OrderItems = []*OrderLineItem{}
		}
		query.withFKs = true
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "order_order_items" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "order_order_items" returned %v for node %v`, *fk, n.ID)
			}
